...
prop.table(table(
  is.na(raw.data.sub$toss_decision),
  !is.na(raw.data.sub$toss_decision)
)) # 21%
...
raw.data.sub$team_a_venue <- NA
raw.data.sub$team_b_venue <- NA

raw.data.sub$team_a <- as.character(raw.data.sub$team_a)
raw.data.sub$team_b <- as.character(raw.data.sub$team_b)
raw.data.sub$venue_country <-
  as.character(raw.data.sub$venue_country)

set.home.match <- function(team_col, venue_col, team_venue_col) {
  for (i in 1:length(team_col)) {
    if (team_col[i] == venue_col[i])
      team_venue_col[i] <- "home"
    else
      team_venue_col[i] <- "away"
  }
  team_venue_col
}

raw.data.sub$team_a_venue <-
  set.home.match(raw.data.sub$team_a,
                 raw.data.sub$venue_country,
                 raw.data.sub$team_a_venue)
raw.data.sub$team_a_venue <- as.factor(raw.data.sub$team_a_venue)


raw.data.sub$team_b_venue <-
  set.home.match(raw.data.sub$team_b,
                 raw.data.sub$venue_country,
                 raw.data.sub$team_b_venue)
raw.data.sub$team_b_venue <- as.factor(raw.data.sub$team_b_venue)
...
raw.data.sub <-
  raw.data.sub %>% select(
    ...
    date,
    series,
    team_a_venue:team_b_venue,
    toss_winner:first_innings_total,
    day_night,
    rain:duckworth_lewis,
    winner
  )
